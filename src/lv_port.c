...
lv_indev_t *lvgl_port_add_touch(const lvgl_port_touch_cfg_t *touch_cfg)
{
    ...
    touch_ctx->indev_drv.user_data = touch_ctx;
        ...
        // ---- Gesture/scroll tuning: make drag scroll instead of click ----
    touch_ctx->indev_drv.scroll_limit = 20;        // px move before LVGL starts scrolling (raise if still toggling)
    touch_ctx->indev_drv.scroll_throw = 30;        // momentum after release
    touch_ctx->indev_drv.long_press_time = 300;    // optional: reduce accidental long-press on tiles
