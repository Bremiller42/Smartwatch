...
{   
    ...
    if (s == g_ui_current) return;  // optional: avoid redundant loads

    // ---- LEAVING old screen: pause things that should not run off-screen ----
    ...
    if (s == UI_HOME) {
        ...
        if (g_notif_dirty) ui_notif_refresh_async();
